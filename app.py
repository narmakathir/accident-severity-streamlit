...
def load_data(uploaded_file=None):
    ...
    if target_col is None:
        st.error("⚠️ Could not find target column in the dataset. Expected one of: " + ", ".join(possible_targets))
        st.stop()
...
if page == "Home":
    ...
    if 'Injury Severity' in df.columns:
        ...
        st.write(df['Injury Severity'].value_counts())
...
# --- Data Analysis ---
    ...
    if 'Injury Severity' in df.columns:
        ...
        ax.set_xlabel('Injury Severity Level', color='white')
        ax.set_ylabel('Count', color='white')
        ax.tick_params(colors='white')
        st.pyplot(fig)
    else:
        st.warning("⚠️ Injury Severity column not found - cannot display distribution")
    st.divider()
...
# --- Admin ---
    ...
    if uploaded_file is not None:
        try:
            ...
            required_columns = ['Injury Severity', 'Weather', 'Light']  # Basic validation
